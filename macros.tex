...
\newcommand{\MathReplaceOp}{\mathds{K}({#1})}
...
\newcommand{\UTRApp}{\UnknownTypeRepair{Application}}
...
\newcommand{\deltaIndTypeOp}{\op{\delta_{\text{InductiveType}}}}
\newcommand{\deltaInductiveType}[5]{
  \deltaIndTypeOp\op{(} #1\op{,}\ #2\op{,}\ #3\op{,}\ #4\op{) =\ } \out{#5}
}

\newcommand{\deltaConstructorOp}{\op{\delta_{\text{ConstructorType}}}}
\newcommand{\deltaConstructor}[8]{ \deltaConstructorOp\op{(}
\diff{#1}{#2} \op{,} \diff{#3}{#4} \op{,} \diff{#5}{#6} \op{,}\ #7 \op{) =\ }
\out{#8} }

\newcommand{\deltaEliminatorOp}{\op{\delta_{\text{EliminatorType}}}}
\newcommand{\deltaEliminatorNameOp}{\op{\delta_{\text{EliminatorName}}}}
\newcommand{\deltaEliminator}[4]{
  \deltaEliminatorOp\op{(} \diff{#1}{#2} \op{,} #3 \op{) =\ } \out{#4}
}
