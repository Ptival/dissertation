...
Nested $\Pi$-types can depend on the values of previously-quantified variables.
We call a \define{$\Pi$-telescope} any sequence of $\Pi$-types directly nested
within one another.  We can summarize them in a list of bindings and their
types, where each type is allowed to, but does not have to, depend on previous
bindings.  For instance, the type:
...
Our work focuses mostly on programs built using a \define{proof assistant}.  A
...
properties of those structures.  By \define{mechanized}, we mean that the
...
or \Language{CoC}.  While it allows itself to be extended TODO TODO TODO
