...
\paragraph{Output types} Note that the type of $\out{\delta_{v}}$
...
\begin{Rules}{lookup-diff-dispatch}{ Diff lookup rules (local context and global environment) }
...
Figure~\ref{lookup-diff-context} shows how we proceed to lookup for a diff in
the local context.
